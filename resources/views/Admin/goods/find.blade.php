		...
		<form action="{{url('/goods/update/'.$res->goods_id)}}"  method="post" enctype="multipart/form-data">
			...
			<table border="1">
				...
				<tr>
					<td>商品详情</td>
					<td>
						<textarea name="goods_desc" id="" cols="30" rows="10">{{$res->goods_desc}}</textarea>
				...
				<tr>
					<td>商品货号</td>
					<td>
						<input type="text" name="goods_score" value="{{$res->goods_score}}">
				...
				<tr>
					<td>商品主图</td>
					<td>
						<img src="http://uploads.1910.com/{{$res->goods_img}}" width="50px"><input type="file" name="goods_img">
					</td>
				</tr>
				<tr>
					<td>商品相册</td>
					<td>
						@foreach($res["goods_imgs"] as $error)
						<img src="http://uploads.1910.com/{{$error}}" width="50px">
						@endforeach
				...
				<tr>
					<td>是否新品</td>
					<td>
						<input type="radio" name="is_nwe" value="1" {{$res->is_nwe == 1 ? "checked" : ""}}>是
						<input type="radio" name="is_nwe" value="2" {{$res->is_nwe == 2 ? "checked" : ""}}>否
				...
				<tr>
					<td>是否精品</td>
					<td>
						<input type="radio" name="is_hot" value="1" {{$res->is_hot == 1 ? "checked" : ""}}>是
						<input type="radio" name="is_hot" value="2" {{$res->is_hot == 2 ? "checked" : ""}}>否
				...
				<tr>
					<td>是否是幻灯片推荐</td>
					<td>
						<input type="radio" name="is_slide" value="1" {{$res->is_slide == 1 ? "checked" : ""}}>是
						<input type="radio" name="is_slide" value="2" {{$res->is_slide == 2 ? "checked" : ""}}>否
				...
				<tr>
					...
					<td>
						<select name="blog_id" id="">
							...
							<option value="{{$error->blog_id}}" {{$res->blog_id == $error->blog_id ? "selected" : ""}}>{{$error->blog_name}}</option>
							@endforeach
						</select>
				...
				<tr>
					...
					<td>
						<select name="cate_id" id="">
							...
							<option value="{{$error->cate_id}}" {{$res->cate_id == $error->cate_id ? "selected" : ""}}>{{str_repeat("+",$error["level"]*2)}}{{$error->cate_name}}</option>
							@endforeach
						</select>
